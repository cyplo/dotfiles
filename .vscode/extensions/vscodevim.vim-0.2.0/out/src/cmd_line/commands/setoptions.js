            ...
            switch (this._arguments.operator) {
                ...
                case SetOptionOperator.Subtract:
                    if (typeof this._arguments.value === "number") {
                        ...
                        let initialValue = configuration_1.Configuration.getInstance()[this._arguments.name];
                        configuration_1.Configuration.getInstance()[this._arguments.name] = initialValue.split(this._arguments.value).join('');
                    }
                    break;
                case SetOptionOperator.Info:
                    let value = configuration_1.Configuration.getInstance()[this._arguments.name];
                    if (value === undefined) {
                        yield util.showError(`E518 Unknown option: ${this._arguments.name}`);
                    }
                    else {
                        yield util.showInfo(`${this._arguments.name}=${value}`);
                    }
                    break;
                default:
                    break;
