    ...
    exec(position, vimState) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let i = 1; i < timesToRepeat; i++) {
                yield textEditor_1.TextEditor.insert(vimState.replaceState.newChars.join(""), position);
                position = position_1.Position.FromVSCodePosition(vscode.window.activeTextEditor.selection.start);
            }
...
let CommandReplaceAtCursor = class CommandReplaceAtCursor extends BaseCommand {
    ...
    execCount(position, vimState) {
        return __awaiter(this, void 0, void 0, function* () {
            let timesToRepeat = this.canBePrefixedWithCount ? vimState.recordedState.count || 1 : 1;
            vimState.currentMode = mode_1.ModeName.Replace;
            vimState.replaceState = new modeHandler_1.ReplaceState(position, timesToRepeat);
            return vimState;
        });
    }
};
...
let CommandCtrlPagedown = class CommandCtrlPagedown extends CommandTabNext {
    constructor(...args) {
        super(...args);
        this.keys = ["<C-pagedown>"];
    }
};
CommandCtrlPagedown = __decorate([
    RegisterAction
], CommandCtrlPagedown);
...
let CommandCtrlPageup = class CommandCtrlPageup extends CommandTabPrevious {
    constructor(...args) {
        super(...args);
        this.keys = ["<C-pageup>"];
    }
};
CommandCtrlPageup = __decorate([
    RegisterAction
], CommandCtrlPageup);
